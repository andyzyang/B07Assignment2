...
public class CdTest {
  ...
  @Test
  public void testcdintoroot() {
    Environment env = new Environment();
    new Cd(env).execute(new Argument("/"));
    new Pwd(env).execute(new Argument(""));
    assertEquals("/" + System.lineSeparator(), output.toString());
  }

  @Test
  public void testcdintorootusingdot() {
    Environment env = new Environment();
    new Cd(env).execute(new Argument("."));
    new Pwd(env).execute(new Argument(""));
    assertEquals("/" + System.lineSeparator(), output.toString());
  ...
  @Test
  public void testcdintodirectoryusingrelative() {
    Environment env = new Environment();
    new Mkdir(env).execute(new Argument("dir1"));
    new Cd(env).execute(new Argument("dir1"));
    new Pwd(env).execute(new Argument(""));
    assertEquals("/dir1" + System.lineSeparator(), output.toString());
  }

  @Test
  public void testcdintodirectoryusingabsolute() {
    Environment env = new Environment();
    new Mkdir(env).execute(new Argument("dir1"));
    new Cd(env).execute(new Argument("/dir1"));
    new Pwd(env).execute(new Argument(""));
